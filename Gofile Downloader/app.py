...
def get_env_var(
    ...
    default: Any = None,
    required: bool = False,
    type_func: Optional[Callable[[str], Any]] = None,
) -> Any:
    """
...
DEFAULT_CONFIG = {
    ...
    "auth": {"enabled": False, "username": "admin", "password": "change-me-in-production"},
    "csrf": {"enabled": True, "time_limit": 3600},  # 1 hour
}
...
config["auth"]["enabled"] = get_env_var(
    "AUTH_ENABLED", auth_enabled, False, lambda x: x.lower() == "true"
)
config["auth"]["username"] = get_env_var(
    "AUTH_USERNAME", config["auth"].get("username", DEFAULT_CONFIG["auth"]["username"])
)
config["auth"]["password"] = get_env_var(
    "AUTH_PASSWORD", config["auth"].get("password", DEFAULT_CONFIG["auth"]["password"])
)
...
def authenticate():
    ...
    return Response(
        "Authentication required\n", 401, {"WWW-Authenticate": 'Basic realm="gofile-dl"'}
    )
...
def download_task(
    url: str, directory: Optional[str], password: Optional[str], task_id: str
) -> None:
    """
    ...
    folder_pattern = download_tasks[task_id].get(
        "folder_pattern", "⭐NEW FILES in |NEW FILES in |⭐"
    )
...
def pause(task_id):
    ...
    return jsonify({"task_id": task_id, "paused": task["paused"], "status": status})
...
def browse():
    ...
    dirs = sorted(
        item for item in os.listdir(target_dir) if os.path.isdir(os.path.join(target_dir, item))
    )
...
def index():
    if request.method == "POST":
        ...
        return redirect(url_for("start_download", url=url, directory=directory, password=password))
