    ...
    def test_content_access(self, url: str, password: Optional[str] = None) -> bool:
        ...
        try:
            response = requests.get(
                ...
                headers={"Authorization": f"Bearer {self.token}", "X-Website-Token": self.wt},
                params=params,
                timeout=10,
            )
            response.raise_for_status()
...
Note: Never commit real URLs or passwords to version control!
    ...
    parser.add_argument(
        "--url", type=str, help="GoFile URL to test (format: https://gofile.io/d/CONTENT_ID)"
    )
    parser.add_argument("--password", type=str, help="Password for protected content (optional)")
    parser.add_argument(
        "--no-password", action="store_true", help="Skip password prompt for non-protected content"
    )
    ...
    if not password and not args.no_password:
        try:
            password_input = input(
                "Enter password (press Enter if content is not password-protected): "
            ).strip()
